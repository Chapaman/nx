defmodule Nx.Defn.Evaluator do
  ...
  The following options are specific to this compiler:
    ...
    * `:debug_options` - a keyword list of options for
      ...
      * `:save_as` - format for saved files when `:save_path` is provided.
        Supported values are `:txt` (default) and `:exs`.
  ...
  defp debug_node(%Nx.Tensor{data: expr}, res, state) do
    ...
    opts = Keyword.validate!(opts, [:inspect_limit, :save_path, :save_as])
      ...
      :ok =
        if save_path do
          case opts[:save_as] do
            :exs -> save_node_info_to_exs(save_path, id, expr, res, inspect_limit)
            _ -> save_node_info_to_file(save_path, id, node_info)
          end
        else
          IO.puts(node_info)
        end
  ...
  defp format_node_info(%Expr{id: id, op: op, args: args}, res, inspect_limit) do
    inspect_limit = normalize_inspect_limit(inspect_limit)
    args =
      Enum.map(
        args,
  ...
  defp save_node_info_to_exs(save_path, id, %Expr{} = expr, res, inspect_limit) do
    inspect_limit = normalize_inspect_limit(inspect_limit)
    sanitized_id = inspect(id) |> String.replace(~r/[^a-zA-Z0-9_]/, "_")
    file = Path.join(save_path, "node_#{sanitized_id}.exs")

    term = build_node_term(expr, res, inspect_limit)

    contents =
      [
        "# Autogenerated by Nx.Defn.Evaluator (debug save_as: :exs)\n",
        "# Evaluate this file to get the node term bound to variable `node`\n\n",
        "node = ",
  ...
  defp build_node_term(%Expr{id: id, op: op, args: args}, res, inspect_limit) do
    inspect_limit = normalize_inspect_limit(inspect_limit)
    %{
      id: safe_expr_id_to_string(id),
      op: op,
      args: Enum.map(args, &serialize_arg(&1, inspect_limit)),
      result: serialize_result(res, inspect_limit)
    }
  end

  defp serialize_arg(%Nx.Tensor{data: %Expr{} = e} = t, _inspect_limit) do
    %{
      kind: :expr_tensor,
      id: safe_expr_id_to_string(e.id),
      op: e.op,
      type: t.type,
      shape: t.shape,
      names: t.names
  ...
  defp serialize_arg(%Nx.Tensor{} = t, inspect_limit) do
    inspect_limit = normalize_inspect_limit(inspect_limit)
    %{
      kind: :tensor,
      type: t.type,
      shape: t.shape,
      names: t.names,
      preview: inspect(t, limit: inspect_limit)
    }
  end

  defp serialize_arg(other, inspect_limit) do
    inspect_limit = normalize_inspect_limit(inspect_limit)
    %{
      kind: :other,
      value: inspect(other, limit: inspect_limit)
    }
  end

  defp serialize_result(%Nx.Tensor{} = t, inspect_limit) do
    inspect_limit = normalize_inspect_limit(inspect_limit)
    %{
      kind: :tensor,
      type: t.type,
  ...
  defp serialize_result(other, inspect_limit) do
    inspect_limit = normalize_inspect_limit(inspect_limit)
    %{
      kind: :other,
      value: inspect(other, limit: inspect_limit)
  ...
  defp expr_id_to_string(id) do
    :erlang.ref_to_list(id)
    |> List.to_string()
    |> String.replace(["#Ref<", ">"], "")
  end

  defp safe_expr_id_to_string(id) when is_reference(id), do: expr_id_to_string(id)
  defp safe_expr_id_to_string(_), do: nil

  defp normalize_inspect_limit(nil), do: 50
  defp normalize_inspect_limit(limit) when limit == :infinity or is_integer(limit), do: limit
end
